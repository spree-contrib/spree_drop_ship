  ...
  module Admin
    class DropShipOrdersController < Spree::Admin::ResourceController
      ...
      def confirm
        ...
        if @order.confirm!
          flash[:notice] = Spree.t('spree.admin.drop_ship_orders.confirm.success', number: @order.id)
        end
      ...
      def deliver
        @order = DropShipOrder.accessible_by(current_ability, :update).find(params[:id])
        if @order.deliver!
          flash[:notice] = Spree.t('spree.admin.drop_ship_orders.deliver.success', number: @order.id)
        end
        redirect_to spree.edit_admin_drop_ship_order_path(@order)
      end
